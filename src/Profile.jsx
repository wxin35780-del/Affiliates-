...
import { Portrait, StatusBadge, SectGlyph, TierPips, Meter, Btn, OrnRule, DualHead, Seal } from './ui';
import { skillsFor, attrsFor, memoryFor } from './protoData';
...
function TabOverview({ a, isMobile }) {
...
function TabTask({ a, onAssign, isMobile }) {
      ...
      <Panel style={{ background: active?'linear-gradient(120deg,#1c2417,#13160f)':'var(--ink-1)', border:`1px solid ${active?'var(--st-active)':'var(--ink-line)'}` }}>
        ...
        {active ? (
          ...
          <div style={{ marginTop:18, textAlign:'center', padding:'24px 0' }}>
            ...
            <Btn kind="gold" sm style={{ marginTop:14 }} onClick={()=>onAssign&&onAssign(a)}>มอบหมายภารกิจ</Btn>
          </div>
        )}
      </Panel>
      <Panel>
...
function TabBtn({ label, cjk, active, onClick }) {
  return (
...
export function ProfileView({ agent:a, tab, setTab, onBack, onEdit, onAssign, isMobile }) {
  const px = isMobile ? '16px' : '32px';
  return (
    <div>
      <div style={{ position:'relative', padding:`18px ${px} 0`, borderBottom:'1px solid var(--ink-line)',
        ...
        <div style={{ display:'flex', gap: isMobile?14:22, flexWrap: isMobile?'wrap':'nowrap' }}>
          <div style={{ position:'relative', flex:'0 0 auto' }}>
            <Portrait glyph={a.glyph} accent={a.accent} label="ภาพจอมยุทธเต็มตัว" slotId={`slot-${a.id}`}
              style={{ width: isMobile?100:148, height: isMobile?124:182, borderRadius:4, border:'1px solid var(--gold-deep)' }} />
            <div style={{ position:'absolute', bottom:-10, right:-10 }}><Seal text={a.glyph} size={isMobile?32:42} /></div>
          </div>
          <div style={{ flex:1, paddingTop:4, minWidth:0 }}>
            ...
            <h1 className="jh-disp" style={{ fontSize: isMobile?22:33, fontWeight:700, color:'var(--gold-hi)', marginTop:8 }}>{a.alias}</h1>
            <p style={{ fontSize: isMobile?12:14, color:'var(--tx-dim)', marginTop:3 }}>{a.real} · {a.role}</p>
            {!isMobile && <p style={{ fontSize:13, color:'var(--tx-faint)', marginTop:11, maxWidth:620, lineHeight:1.55 }}>{a.persona}</p>}
            <div style={{ display:'flex', gap: isMobile?12:24, marginTop:10, alignItems:'center', flexWrap:'wrap' }}>
              <span style={{ fontSize:13, color:'var(--tx-dim)' }}>เลเวล <b style={{ color:'var(--gold)', fontSize:18, fontFamily:'var(--font-mono)' }}>{a.level}</b></span>
              <span style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'var(--gold-hi)' }}>{TIERS[a.tier]} <TierPips tier={a.tier} /></span>
              {!isMobile && <span style={{ fontSize:13, color:'var(--tx-dim)' }}>พลัง <b style={{ color:'var(--gold)', fontSize:16, fontFamily:'var(--font-mono)' }}>{a.power.toLocaleString()}</b></span>}
            </div>
            {isMobile && (
              <div style={{ display:'flex', gap:8, marginTop:12 }}>
                <Btn kind="gold" sm onClick={()=>onAssign&&onAssign(a)}>⚔ มอบหมาย</Btn>
                <Btn kind="ghost" sm onClick={()=>onEdit&&onEdit(a)}>แก้ไข</Btn>
              </div>
            )}
          </div>
          {!isMobile && (
        ...
        <div style={{ display:'flex', gap:4, marginTop:16, overflowX:'auto' }} className="jh-scroll">
          <TabBtn label="สำนวน" cjk="總覽" active={tab==='overview'} onClick={()=>setTab('overview')} />
          <TabBtn label="วิชา" cjk="武學" active={tab==='skills'} onClick={()=>setTab('skills')} />
          <TabBtn label="คลังคัมภีร์" cjk="記憶" active={tab==='memory'} onClick={()=>setTab('memory')} />
          <TabBtn label="ภารกิจ" cjk="任務" active={tab==='task'} onClick={()=>setTab('task')} />
        </div>
      </div>
      <div style={{ padding: isMobile?`20px ${px} 32px`:`26px ${px} 40px` }}>
        {tab==='overview' && <TabOverview a={a} isMobile={isMobile} />}
        {tab==='skills' && <TabSkills a={a} isMobile={isMobile} />}
        {tab==='memory' && <TabMemory a={a} isMobile={isMobile} />}
        {tab==='task' && <TabTask a={a} onAssign={onAssign} isMobile={isMobile} />}
      </div>
    </div>
  );
}
